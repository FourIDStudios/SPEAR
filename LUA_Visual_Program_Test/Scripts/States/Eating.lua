...
State.Benefits = {
	Energy = .025,      -- Small energy boost from food
	Resources = -1.0,   -- Spending resources on food
	Mood = 1.0,        -- Mood improvement from satisfying hunger
	Hunger = 5.0,      -- Significant hunger reduction
	Health = 0.5       -- Small health benefit from nourishment
}
...
function State.Tick(Brain:Utils.Brain) --Our tick function for state logic
	if State.isTransitioning then
		local TargetLocation = workspace.Eat_Location
		...
		Brain.Body.Character:SetAttribute("Eating",true)
